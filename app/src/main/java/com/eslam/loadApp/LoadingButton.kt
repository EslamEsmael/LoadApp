...
class LoadingButton @JvmOverloads constructor(
    ...
    var buttonState: ButtonState by Delegates.observable<ButtonState>(ButtonState.Completed) { _, _, new ->
        when (new) {
            ...
            ButtonState.Loading -> {
                ...
                valueAnimator = ValueAnimator.ofFloat(0f, buttonWidth.toFloat())
                valueAnimator.duration = 4000
                valueAnimator.addUpdateListener { animation ->
                    progressWidth = animation.animatedValue as Float
                    progressCircle = (buttonWidth.toFloat() / 365) * progressWidth
                    invalidate()
                }
            ...
            ButtonState.Completed -> {
                ...
                progressWidth = 0f
                progressCircle = 0f
                buttonTitle = resources.getString(R.string.download)
                invalidate()
            }
        }
    ...
    private fun drawBackgroundColor(canvas: Canvas?) {
        ...
        canvas?.drawRect(0f, 0f, buttonWidth.toFloat(), buttonHeight.toFloat(), paint)
    }

    private fun drawProgressBackground(canvas: Canvas?) {
        paint.color = loadingColor
        canvas?.drawRect(0f, 0f, progressWidth, buttonHeight.toFloat(), paint)
    }

    private fun drawCircleProgress(canvas: Canvas?) {
        canvas?.save()
        canvas?.translate(
            buttonWidth / 2 + textWidth / 2 + circleXOffset,
            buttonHeight / 2 - textSize / 2
        )
    ...
    private fun drawTitle(canvas: Canvas?) {
        ...
        canvas?.drawText(
            buttonTitle,
            buttonWidth / 2 - textWidth / 2,
            buttonHeight / 2 - (paint.descent() + paint.ascent()) / 2,
            paint
        )
    }
